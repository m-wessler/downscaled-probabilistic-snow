...
def openmfd(paths, lset, cdim):
    ...
    if ( (ensemble == 'naefs') & (lset == 'surface') ):
        ...
        # so just swap it for tp with zeros
        orog_save = datasets[0].orog.copy()
...
# DO NOT USE - INACCURATE ESTIMATE OF Tw
# Use wrf-python.wetbulb()
# def calctw(_t, _rh):
#     import warnings
#     warnings.filterwarnings("ignore")
...
def downscale_calc_slr_chunked(lr_swapfile, hr_swapfile, iterp_mode='linear'):
    ...
    for i, lr in enumerate(_lr.groupby('time')):
        ...
        saveset['dqpf_units'] = 'in'
        saveset['snow_units'] = 'in'
        saveset['slr_units'] = 'ratio(qpf/qsf)'
        saveset['wbzh_units'] = 'm'
...
def build_netCDF(xarr, i, tsize, init, fpath, mode):
    ...
    with Dataset(fpath, mode, format=ncformat) as dataset:
        ...
        if mode == 'w':
            ...
            vardat = {}
            varunits = {'dqpf':'inches', 'snow':'inches', 'slr':'ratio', 'wbzh':'meters'}
