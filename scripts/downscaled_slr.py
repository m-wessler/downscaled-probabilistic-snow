...
if __name__ == '__main__':
    ...
    if process_new:
        ...
        p = isobaric.level
        _p = np.ones(t.shape)
        _p = np.array([_p[:, :, i, :, :]*p[i].values for i in range(p.size)]).transpose(1, 2, 0, 3, 4)
        p = t.copy().rename('p')
        p.values = _p
        isobaric['p'] = p
        ...
        qv.values = np.array(mpc.mixing_ratio_from_relative_humidity(
            rh.values/100, t.values*mpu.degC, p.values*mpu.millibar))
        # Repair the dimensions after metpy messes with them
        qv['time'] = isobaric.time
        qv['level'] = isobaric.level
        qv['member'] = isobaric.member
        qv['lat'], qv['lon'] = isobaric.lat, isobaric.lon

        # Finally, we can calculate the wet bulb
        # Need to pass Pa and Kelvin, will return deg C
        tw = wrf.wetbulb(p*100, t+273.15, qv, units='degC')
        # Repair the dimensions after wrf messes with them
        tw['time'] = isobaric.time
        tw['level'] = isobaric.level
        tw['member'] = isobaric.member
        tw['lat'], tw['lon'] = isobaric.lat, isobaric.lon

        print(' ...done\nCalculating height of wet bulb zero...')
        wbz = calcwbz(tw, gh, surface.orog)
